...
static int32_t handle_accept(int fd) {
  ...
  g_data.idle_head.insert_back(&conn->idle_node);
...
static void entry_del(Entry *ent) {
  ...
  size_t set_size = (ent->type == T_ZSET) ? ent->zset.hmap.size() : 0;
...
static void do_keys(std::vector<std::string> &, Buffer &out) {
  out.write_frame_array((uint32_t)g_data.db.size());
  g_data.db.foreach (&cb_keys, (void *)&out);
}
...
static uint32_t next_timer_ms() {
  ...
  if (!g_data.idle_head.empty()) {
    Conn *conn = container_of(g_data.idle_head.next, Conn, idle_node);
