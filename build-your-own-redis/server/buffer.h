...
public:
  ...
  inline void push_back(uint8_t data) { buf.push_back(data); }
  inline void push_back(uint32_t data) { push_back((const uint8_t *)&data, 4); }
  inline void push_back(int64_t data) { push_back((const uint8_t *)&data, 8); }
  inline void push_back(double data) { push_back((const uint8_t *)&data, 8); }

  // append serialized data types to the back
  inline void out_nil() { push_back((uint8_t)TAG_NIL); }
